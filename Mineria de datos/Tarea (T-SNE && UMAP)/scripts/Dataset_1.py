...
COLUMNAS_EXCLUIR = ['label', 'label_name']
COLORES = plt.cm.tab10.colors


def preparar_datos(df, target_col='label_name'):
...
def _graficar(ax, datos, target_col, titulo):
    ...
    ax.legend(title='Tipo de ropa', bbox_to_anchor=(1.02, 1), loc='upper left',
...
def _graficar_3d(ax, datos, target_col, titulo):
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(title='Tipo de ropa', bbox_to_anchor=(1.05, 1), loc='upper left',
              fontsize=6, title_fontsize=7)


def acp(df, target_col='label_name', n_components=2, random_state=42, graficar=True):
    ...
    if graficar:
        if n_components == 3:
            ...
            _graficar(ax, resultado, target_col, 'ACP')
        plt.tight_layout()
        plt.show()
...
def tsne(df, target_col='label_name', n_components=2, perplexity=30, random_state=42, graficar=True):
    X_scaled, y = preparar_datos(df, target_col)
    ...
    if graficar:
        if n_components == 3:
            ...
            fig, ax = plt.subplots(figsize=(6, 5))
            _graficar(ax, resultado, target_col, 't-SNE')
        plt.tight_layout()
        plt.show()
...
def umap(df, target_col='label_name', n_components=2, lista_vecinos=None, random_state=42, graficar=True):
    ...
    if graficar:
        if n_components == 3:
            ...
            fig, ax = plt.subplots(figsize=(6, 5))
            _graficar(ax, mejor_resultado, target_col, 'UMAP')
        plt.tight_layout()
        plt.show()
...
def graficar_comparacion(resultado_acp, resultado_tsne, resultado_umap, target_col='label_name'):
    ...
    _graficar(ax1, resultado_acp, target_col, 'ACP')
    _graficar(ax2, resultado_tsne, target_col, 't-SNE')
    _graficar(ax3, resultado_umap, target_col, 'UMAP')

    plt.tight_layout()
    plt.show()


def graficar_comparacion_3d(resultado_acp, resultado_tsne, resultado_umap, target_col='label_name'):
